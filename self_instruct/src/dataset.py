...
class ChatDataset(Dataset):
    ...
    def convert_record(self, record):
        ...
        if len([l for l in labels if l != -100]) == 0:
            return None
        ...
        if input_ids[-1] == self.tokenizer.eos_token_id and input_ids[-4] == self.tokenizer.eos_token_id:
        ...
        if input_ids.size(0) > self.max_tokens_count:
            return None
...
class LineDataset(Dataset):
    ...
    def get_tokens(self, text):
        return self.tokenizer(
            ...
            padding=False,
            truncation=True,
            max_length=self.max_tokens_count-2
        )["input_ids"]
    ...
    def convert_record(self, record):
        ...
        for message, role in conversation.iter_messages():
            ...
            labels.extend(message_labels)
        '''
        input_ids = self.get_tokens(record['text'] + self.tokenizer.eos_token)
